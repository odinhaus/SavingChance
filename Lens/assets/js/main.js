			...
			// Touch gestures.
				_.$main
	                ...
	                .on('scroll', function (event) {
	                    // handle thumbnails scrolling
	                    var i = _.$main.scrollTop();
	                    if (i == 0)
	                    {
	                        _.initViewer(true);
	                    }
			...
			_.$thumbnails.children()
				.each(function() {
					...
					// Slide.
						...
						// Create elements.
						s.$slide = $('<div class="slide"><div class="caption"></div><div class="image"></div><div class="content"></div></div>');
				        ...
				        // Content.
	 						s.$slideContent = s.$slide.find('.content');

						// Move everything *except* the thumbnail itself to the caption.
							$this.children().not($thumbnail).not($content)
								.appendTo(s.$slideCaption); // moves the source elements, not clones!!!!

				        // Move everything *except* the thumbnail itself to the caption.
							$content.appendTo(s.$slideContent); // moves the source elements, not clones!!!!
